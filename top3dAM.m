...
xPhys = x; 
baseplate = 'S'; % AMFILTER CALL 1: %%%%%%%%%
xPrint = AMFilter3D(xPhys, baseplate); %%%%%%%%%
loop = 0; 
change = 1;
% START ITERATION
while change > tolx && loop < maxloop
    loop = loop+1;
    % FE-ANALYSIS
    sK = reshape(KE(:)*(Emin+xPrint(:)'.^penal*(E0-Emin)),24*24*nele,1);
    K = sparse(iK,jK,sK);
    K = (K+K')/2;
    U(freedofs,:) = K(freedofs,freedofs)\F(freedofs,:);
    % OBJECTIVE FUNCTION AND SENSITIVITY ANALYSIS
    ce = reshape(sum((U(edofMat)*KE).*U(edofMat),2),[nely,nelx,nelz]);
    c = sum(sum(sum((Emin+xPrint.^penal*(E0-Emin)).*ce)));
    dc = -penal*(E0-Emin)*xPrint.^(penal-1).*ce;
    dv = ones(nely,nelx,nelz);
    % Transform sensitivities dc and dv before filtering (AMFILTER CALL 2)
    [xPrint, dc, dv] = AMFilter3D(xPhys, baseplate, dc, dv); %%%%%%%%
    ...
    while (l2-l1)/(l1+l2) > 1e-3
        ...
        xPhys(:) = (H*xnew(:))./Hs;
        xPrint = AMFilter3D(xPhys, baseplate); %%%%%%%%%
        if sum(xPrint(:)) > volfrac*nele, l1 = lmid; else l2 = lmid; end
    end
